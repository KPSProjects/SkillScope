...
from src.config import (
    ...
    WEAK_LABELLED_REED_LARGE_V3_PREVIEW_PATH,
)
...
def looks_like_requirement_chunk(chunk):
    chunk_norm = normalise_text(chunk)
    ...
    return any(signal in chunk_norm for signal in positive_signals)
...
def is_strong_skill_phrase(phrase):
    ...
    if phrase_norm in banned_phrases:
        return False
    ...
    if len(phrase_norm.split()) >= 2:
        # reject very broad multi-word phrases
        weak_multi_patterns = [
            "work in",
            "working in",
            "experience of",
        ...
        if any(phrase_norm.startswith(p) for p in weak_multi_patterns):
            return False
        return True
...
def build_weak_labelled_rows(df, esco_phrases, max_rows=500):
    ...
    print("Starting final strict v3 ESCO weak labelling...")
    ...
    for i, (_, row) in enumerate(working_df.iterrows(), start=1):
        ...
        for chunk in chunks:
            ...
            # for descriptions, only keep chunks that really look like requirement text
            if source_field == "job_description" and not looks_like_requirement_chunk(chunk):
                continue
            ...
            output_rows.append(
                {
                    ...
                    "source": f"reed_large_weak_v3_{source_field}",
                }
            )
...
def save_preview(rows, preview_path):
    ...
    with open(preview_path, "w", encoding="utf-8") as file:
        file.write("WEAK LABELLED REED LARGE V3 PREVIEW\n")
        file.write("===================================\n\n")
...
def run_build_weak_labels_v3():
    overall_start = time.time()
    print("Starting v3 weak label generation from cleaned Reed large data...")
    ...
    save_jsonl(weak_rows, WEAK_LABELLED_REED_LARGE_V3_PATH)
    save_preview(weak_rows, WEAK_LABELLED_REED_LARGE_V3_PREVIEW_PATH)
    save_end = time.time()

    print(f"Save time: {save_end - save_start:.2f} seconds")
    print(f"Saved weak-labelled JSONL to: {WEAK_LABELLED_REED_LARGE_V3_PATH}")
    print(f"Saved preview to: {WEAK_LABELLED_REED_LARGE_V3_PREVIEW_PATH}")
