...
from src.config import (
    ...
    WEAK_LABELLED_REED_LARGE_V2_PATH,
    WEAK_LABELLED_REED_LARGE_V2_PREVIEW_PATH,
)
...
def build_weak_labelled_rows(df, esco_phrases, max_rows=500):
    ...
    print("Starting stricter v2 ESCO weak labelling...")
    ...
    for i, (_, row) in enumerate(working_df.iterrows(), start=1):
        ...
        for chunk in chunks:
            ...
            if span_count < 2 and labelled_token_count < 4:
                continue
            ...
            output_rows.append(
                {
                    ...
                    "source": f"reed_large_weak_v2_{source_field}",
                }
            )
...
def save_preview(rows, preview_path):
    ...
    with open(preview_path, "w", encoding="utf-8") as file:
        file.write("WEAK LABELLED REED LARGE V2 PREVIEW\n")
        file.write("===================================\n\n")
...
def run_build_weak_labels_v2():
    overall_start = time.time()
    print("Starting v2 weak label generation from cleaned Reed large data...")
    ...
    save_jsonl(weak_rows, WEAK_LABELLED_REED_LARGE_V2_PATH)
    save_preview(weak_rows, WEAK_LABELLED_REED_LARGE_V2_PREVIEW_PATH)
    save_end = time.time()

    print(f"Save time: {save_end - save_start:.2f} seconds")
    print(f"Saved weak-labelled JSONL to: {WEAK_LABELLED_REED_LARGE_V2_PATH}")
    print(f"Saved preview to: {WEAK_LABELLED_REED_LARGE_V2_PREVIEW_PATH}")
