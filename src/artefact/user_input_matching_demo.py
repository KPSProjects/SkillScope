...
def ask_for_file_path(prompt_text):
...
def get_user_input_text():
    ...
    print("1 - Enter file paths")
    print("2 - Paste raw text directly")

    choice = input("Enter 1 or 2: ").strip()

    if choice == "1":
        job_input_path = ask_for_file_path("Enter the path to the job advert text file: ")
        cv_input_path = ask_for_file_path("Enter the path to the CV text file: ")

        job_text = load_text_file(job_input_path)
        cv_text = load_text_file(cv_input_path)

    elif choice == "2":
        job_text = ask_for_multiline_text("Enter the job advert text")
        cv_text = ask_for_multiline_text("Enter the CV text")
...
def run_user_input_matching_demo():
    print("Running user input CV to job matching demo...")

    skill_dictionary = load_active_skill_dictionary()

    job_text, cv_text = get_user_input_text()
